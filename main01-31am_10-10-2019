...
class MyApp(QtWidgets.QMainWindow, Ui_Form):
    ...
    def out_btnClicked(self):
        try:
            out_path = str(QFileDialog.getExistingDirectory(
        ...
        for a_vidsub in vidsub_lst:
            ...
            for a_sub in subs_vid_lst:
                ...
                if len(a_sub_split) in (2, 3):
                    sub_name.append('--language "0:' +
                                    option_language + '" "' + a_sub + '"')
                # name.order.team.ext
                elif len(a_sub_split) == 4:
                    sub_name.append('--language "0:' + option_language +
                                    '" --track-name "0:' + a_sub_split[2] + '" "' + a_sub + '"')
                ...
                # name.order.team.delay.lang.ext
                    ...
                    sub_name.append('--language "0:' + a_sub_split[4].lower(
                    ) + '" --track-name "0:' + a_sub_split[2] + '" --sync "0:' + a_sub_split[3] + '" "' + a_sub + '"')
            ...
            option_code = '" --forced-track "0:' + option_forced_track + '" --default-track "0:' + \
                option_default_track + '" --track-name "0:' + \
                option_track_name + '" --sync "0:' + option_delay + '" '
            #print(self.mkvmerge_path +' -o "'+ full_dest_vid_path +'" "' + vid_name + option_code + sub_name)
            returncode = subprocess.Popen(self.mkvmerge_path + ' -o "' + full_dest_vid_path + '" "' + vid_name +
                                          option_code + sub_name, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            ############################################
    ...
    def showdialog(self):
        ...
        msg.setDetailedText(
            "Please check your Path Folder again. and for Better Result use the Search button.")
